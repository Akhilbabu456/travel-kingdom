...
import { ArrowRight, Award, ShieldCheck, Sparkles, Compass, Heart, ChevronLeft, ChevronRight, Quote, Star, MapPin } from "lucide-react";
    ...
    <section className="relative isolate min-h-[100svh] overflow-hidden">
      <AnimatePresence mode="sync">
        <motion.div key={i} className="absolute inset-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.4 }}>
          <motion.img style={{ y, scale }} src={heroSlides[i].image} alt="" className="absolute inset-0 h-full w-full object-cover" />
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-7xl flex-col justify-end px-5 pb-12 pt-40 lg:px-8 lg:pb-20">
        ...
        <div className="mt-10 flex flex-wrap items-center gap-4">
          ...
          <Link href="/contact" className="inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/5 px-6 py-3.5 text-sm font-medium text-white backdrop-blur transition hover:bg-white/10">
            Plan a Custom Trip
          </Link>
  ...
  return (
    <section className="mx-auto max-w-7xl px-5 pt-28 lg:px-8">
      <div className="flex items-end justify-between gap-6">
        <SectionHeading eyebrow="Featured Destinations" title="Places We Love & Recommend" sub="A curated atlas of escapes — hand-tested by our team, scored on beauty, safety, and comfort." />
...
function TrendingPackages() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-5 pt-28 lg:px-8">
      <SectionHeading eyebrow="Trending Now" title="Holidays Our Travellers Are Loving" sub="Trending curated tour packages combining seamless flights, verified hotels, and local sightseeing." />
...
function VisaFreeDestinations() {
  ...
  useEffect(() => {
    fetchLocations().then((res) => {
      ...
      const visaFreeKeywords = ["maldives", "bhutan", "bali", "thailand", "indonesia", "nepal", "sri lanka"];
      const filtered = res.filter(d => 
        visaFreeKeywords.some(kw => d.name.toLowerCase().includes(kw) || d.country.toLowerCase().includes(kw))
      );
  ...
  return (
    <section className="mx-auto max-w-7xl px-5 pt-28 lg:px-8">
      <div className="flex items-end justify-between gap-6">
        ...
        />
        <Link href="/destinations?category=Beach" className="hidden shrink-0 items-center gap-1 text-sm font-medium text-primary hover:underline md:inline-flex">Explore More <ArrowRight className="h-4 w-4" /></Link>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <p className="mt-6 text-xs text-muted-foreground italic">
            * Note: For Indian passport holders, Singapore is not a visa-free country. E-visa applications are required prior to departure. We provide full visa processing support.
          </p>
        </>
      )}
    </section>
...
function FindYourPerfectTrip() {
  const themes = [
    { name: "Honeymoon", desc: "Romantic overwater villas & candlelit dinners", count: "12+ Packages", link: "/packages?category=Honeymoon", image: "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=400&q=80" },
    { name: "Family Vacation", desc: "Kid-friendly resorts and worry-free guides", count: "24+ Packages", link: "/packages?category=Family", image: "https://images.unsplash.com/photo-1543269865-cbf427effbad?auto=format&fit=crop&w=400&q=80" },
    { name: "Hidden Gems", desc: "Explore offbeat trails and quiet boutique stops", count: "8+ Packages", link: "/packages?category=Group", image: "https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?auto=format&fit=crop&w=400&q=80" },
    { name: "Luxury Escapes", desc: "Negotiated offline properties and airport perks", count: "18+ Packages", link: "/packages?category=Luxury", image: "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=400&q=80" },
  ];

  return (
    <section className="mx-auto max-w-7xl px-5 pt-28 lg:px-8">
      <SectionHeading eyebrow="Find Your Perfect Trip" title="Exclusive Journeys Curated for You" sub="Select a travel theme and discover packages optimized for your ideal vacation style." />
      <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {themes.map((t, idx) => (
          <Reveal key={t.name} delay={idx * 0.05} className="group relative overflow-hidden rounded-[2rem] border border-border bg-card p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-luxe">
            <div className="relative h-40 overflow-hidden rounded-2xl mb-4">
              ...
              <span className="absolute bottom-3 left-4 text-xs font-semibold text-white/90">{t.count}</span>
            </div>
            <h3 className="font-display text-xl text-foreground group-hover:text-primary transition-colors">{t.name}</h3>
            <p className="mt-2 text-xs leading-relaxed text-muted-foreground">{t.desc}</p>
            <Link href={t.link} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-primary hover:underline">
              Explore packages →
            </Link>
          </Reveal>
        ))}
...
function WhyUs() {
  const items = [
    { icon: Award, t: "Award-winning Concierge", d: "Hand-picked stays, private guides, and exclusive off-peak slots most automated booking portals cannot unlock." },
    { icon: ShieldCheck, t: "Zero-surprise Pricing", d: "Every single charge accounted for upfront — taxes, transfer fees, custom guides, and meals included." },
    { icon: Compass, t: "Designed by Specialists", d: "Personalized itineraries written by seasoned travel designers who have actually visited the locations." },
    { icon: Heart, t: "24/7 WhatsApp Concierge", d: "A dedicated human specialist in your timezone, ready to coordinate check-ins, delays, or emergencies on the fly." },
  ];
  return (
    <section className="relative mt-28 overflow-hidden bg-grain py-24">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <SectionHeading eyebrow="Why Travel Kingdom" title="The Quiet Luxury of a Handcrafted Journey" />
    ...
    <section className="mx-auto mt-28 grid max-w-7xl items-center gap-12 px-5 lg:grid-cols-12 lg:px-8">
      ...
      <Reveal className="lg:col-span-7">
        <div className="relative">
          <img src={heroSlides[1].image} alt="Kerala backwaters at sunrise" className="aspect-[4/3] w-full rounded-[2.5rem] object-cover shadow-luxe" loading="lazy" />
          <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }} className="absolute -right-4 -top-4 hidden rounded-2xl glass p-4 shadow-luxe sm:block">
            <div className="mt-1 font-display text-lg">Priya M.</div>
          </motion.div>
          <motion.div animate={{ y: [0, 10, 0] }} transition={{ duration: 7, repeat: Infinity, ease: "easeInOut" }} className="absolute -bottom-4 -left-4 rounded-2xl bg-primary p-4 text-primary-foreground shadow-glow">
        ...
        {gallery.map((img, idx) => (
          <Reveal key={idx} delay={idx * 0.04}>
            <div className={`relative overflow-hidden rounded-2xl ${idx % 5 === 0 ? "aspect-[4/5]" : "aspect-square"}`}>
              <img src={img} alt="" loading="lazy" className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 hover:scale-110" />
            </div>
          </Reveal>
        ))}
      </div>
  ...
  return (
    <section className="mx-auto mt-28 mb-10 max-w-7xl px-5 lg:px-8">
      <div className="relative overflow-hidden rounded-[2.5rem] bg-secondary px-8 py-20 text-center sm:px-16">
        <img src={heroSlides[0].image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-40" />
        ...
        <div className="relative mx-auto max-w-2xl text-white">
          <Reveal>
            <h2 className="font-display text-4xl leading-tight sm:text-6xl">Your next chapter starts with a single call.</h2>
          </Reveal>
          <Reveal delay={0.15}>
          ...
          <Reveal delay={0.25}>
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <Link href="/contact" className="inline-flex items-center gap-2 rounded-full bg-primary px-7 py-3.5 text-sm font-semibold text-primary-foreground shadow-glow">Plan My Trip <ArrowRight className="h-4 w-4" /></Link>
              <a href="tel:+919860874848" className="inline-flex items-center gap-2 rounded-full border border-white/25 px-6 py-3.5 text-sm font-medium text-white hover:bg-white/10">Talk to a Specialist</a>
            </div>
          </Reveal>
        </div>
      </div>
