...
import { Calendar, Clock, MapPin, Sparkles, ShieldCheck, Mail, Phone, User, Send, CheckCircle2 } from "lucide-react";
...
export default function Page({ params }: PageProps) {
  ...
  const mainImage = (pkg.images && pkg.images.length > 0) ? pkg.images[0] : "https://images.unsplash.com/photo-1502920917128-1aa500764cbd?auto=format&fit=crop&w=1200&q=80";
        ...
        <div className="relative mx-auto w-full max-w-7xl px-5 pb-16 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full glass-dark px-3 py-1 text-[11px] font-medium uppercase tracking-[0.18em] text-white">
              <Clock className="h-3.5 w-3.5 text-primary" /> {pkg.duration} Nights / {pkg.duration + 1} Days
            </div>
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-5 py-20 lg:grid-cols-[1.6fr_1fr] lg:px-8">
        ...
        <div>
          <Reveal>
            ...
            <p className="mt-2 text-sm text-muted-foreground">Every day is customizable to match your timing, transfers, and preferences.</p>
          </Reveal>
          ...
          <div className="mt-10 space-y-8">
            {pkg.itineraries.map((day, idx) => {
              ...
              return (
                <Reveal key={day.id} delay={idx * 0.05} className="relative pl-8 border-l border-border/80 pb-2">
                  <div className="absolute -left-[9px] top-1.5 h-4.5 w-4.5 rounded-full border-2 border-primary bg-background" />
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="text-xs font-bold uppercase tracking-wider text-primary">Day {day.day_number}</span>
                  ...
                  {day.description && <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{day.description}</p>}

                  {/* Sightseeing Items with Thumbnails */}
                  {day.sightseeings && day.sightseeings.length > 0 && (
                    <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                      {day.sightseeings.map((s, sIdx) => {
                        const ssThumbnail = s.images || "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=300&h=200&q=80";
                        return (
                          <div key={sIdx} className="flex gap-3 rounded-2xl border border-border bg-card p-3 shadow-xs">
                            <div className="relative h-16 w-20 shrink-0 overflow-hidden rounded-xl bg-muted">
                              <img src={ssThumbnail} alt={s.title} className="absolute inset-0 h-full w-full object-cover" />
                            </div>
                            <div className="min-w-0">
                              <h4 className="font-semibold text-sm text-foreground truncate">{s.title}</h4>
                              <p className="text-[11px] text-muted-foreground/80 truncate mb-1">{s.city}</p>
                              {s.description && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
                {pkg.inclusions && (
                  <div className="rounded-3xl border border-border bg-card p-6">
                    <h4 className="font-semibold text-foreground text-sm uppercase tracking-wider mb-3 text-primary">Inclusions</h4>
                    <div className="text-sm text-muted-foreground leading-relaxed whitespace-pre-line" dangerouslySetInnerHTML={{ __html: pkg.inclusions }} />
                  </div>
                )}
                {pkg.exclusions && (
                  <div className="rounded-3xl border border-border bg-card p-6">
                    <h4 className="font-semibold text-foreground text-sm uppercase tracking-wider mb-3 text-destructive">Exclusions</h4>
                    <div className="text-sm text-muted-foreground leading-relaxed whitespace-pre-line" dangerouslySetInnerHTML={{ __html: pkg.exclusions }} />
                  </div>
                )}
              </div>
            </Reveal>
        ...
        <aside>
          <div className="sticky top-28 space-y-6">
            <div className="rounded-3xl border border-border bg-card p-6 shadow-luxe">
              {costValue ? (
                <>
                  ...
                  <div className="text-xs text-muted-foreground mt-1">Tax & transfers included · {pkg.duration} Nights</div>
                </>
              ) : (
                <div className="py-2">
                  <div className="text-xs font-semibold uppercase tracking-wider text-primary mb-1">Exclusive Offer</div>
                  <div className="font-display text-2xl leading-snug text-foreground">Enquire Now to Unlock the Best Deals</div>
                  <p className="text-xs text-muted-foreground mt-1">Contact us for private rates and offline resort tariffs.</p>
                </div>
              )}
                ...
                {inquirySent ? (
                  ...
                  >
                    ...
                    <p className="text-xs text-emerald-800">
                      Our travel specialist is checking package schedules and will WhatsApp you within 15 minutes.
                    </p>
                  </motion.div>
                ) : (
                  <form onSubmit={handleInquirySubmit} className="space-y-3.5">
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] uppercase tracking-wider font-semibold text-muted-foreground">Number of Travellers</label>
                      <select 
                        value={travelers}
            ...
            <div className="rounded-3xl border border-border bg-muted/30 p-5 text-xs text-muted-foreground space-y-3.5">
              ...
              <p>We work directly with certified operators, local resort owners, and verified flight desks to assure comfort and FEMA compliance.</p>
            </div>
          </div>
        </aside>
      </section>
