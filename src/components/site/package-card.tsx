...
export function PackageCard({ p }: { p: Package }) {
  return (
    <motion.div
      whileHover={{ y: -6 }}
      className="group overflow-hidden rounded-3xl border border-border bg-card shadow-sm transition-shadow hover:shadow-luxe"
    >
      <Link href={`/packages/${p.slug}`} className="block">
        ...
        <div className="p-5">
          ...
          <h3 className="mt-2 font-display text-xl text-foreground transition-colors group-hover:text-primary">
            {p.title}
          </h3>
