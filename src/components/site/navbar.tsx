  ...
  return (
    ...
    >
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
                ...
                <AnimatePresence>
                  {item.mega && openMega && (
                    ...
                    >
                      ...
                      <Link href="/destinations" className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-primary hover:underline">
        ...
        <button aria-label="Menu" onClick={() => setMobile(true)} className={`lg:hidden grid h-10 w-10 place-items-center rounded-full ${transparent ? "text-white" : "text-foreground"}`}>
          <Menu />
        </button>
      </div>
        ...
        {mobile && (
          ...
          >
            <div className="flex h-20 items-center justify-between px-5">
              <Logo light />
              <button onClick={() => setMobile(false)} className="grid h-10 w-10 place-items-center rounded-full text-white">
                <X />
              </button>
            </div>
            <motion.nav
            ...
            >
              {nav.map((item) => (
                <motion.div key={item.to} variants={{ hidden: { opacity: 0, x: -20 }, show: { opacity: 1, x: 0 } }}>
                  <Link href={item.to} className="block border-b border-white/10 py-4 font-display text-2xl text-white">
                    {item.label}
                  </Link>
                </motion.div>
              ))}
